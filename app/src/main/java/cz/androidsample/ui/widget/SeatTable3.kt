 ...
 * 4:绘图控件改为surfaceView检测绘图效率
 */
class SeatTable3(context: Context, attrs: AttributeSet?, defStyleAttr: Int) :
        SurfaceView(context, attrs, defStyleAttr) , ScaleGestureDetector.OnScaleGestureListener, GestureDetector.OnGestureListener,SurfaceHolder.Callback2 {
    ...
    fun setItemSelected(row:Int,column:Int,selected:Boolean){
        ...
        val rowLayout=seatArray.rowArray[row]
        val columnLayout=seatArray.columnArray[column]
        drawPreviewNode(Canvas(previewBitmap),viewItem.seatView,matrixScaleX,matrixScaleX,
                columnLayout.left,rowLayout.top,columnLayout.right,rowLayout.bottom)
        //重绘界面
        postInvalidate()
        debugLog("setItemSelected:${viewItem.seatView.isSelected} ${viewItem.seatView.isPressed} $selected")
    }
    ...
    inner class PreViewPainter: Handler.Callback {
        ...
        private fun drawReviewBitmap() {
            ...
            if (0 < measuredWidth && 0 < measuredHeight) {
                ...
                //绘完毕后,通知刷新
                postInvalidate()
                debugLog("drawReviewBitmap:${System.currentTimeMillis() - st} $previewWidth $previewHeight $matrixScaleX")
            }
        }
     ...
     * 绘制View管理对象
     */
    inner class ViewItem(adapter: SeatTable3.SeatTableAdapter){
        //座位信息布局
        ...
        fun onMeasured(adapter: SeatTable3.SeatTableAdapter, widthMode: Int, heightMode: Int){
            //一个绘制临时view
    ...
    inner class ViewFlinger:Runnable{
        ...
        fun postOnAnimation() {
            ...
            ViewCompat.postOnAnimation(this@SeatTable3, this)
        }
    }
    /**
     *
     */
    abstract class SeatTableAdapter(val table: SeatTable3){
        /**
         * 获得顶部座位
         */
