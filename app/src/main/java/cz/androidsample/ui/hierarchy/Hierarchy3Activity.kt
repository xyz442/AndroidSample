...
import kotlinx.android.synthetic.main.activity_hierarchy3.*
import org.jetbrains.anko.sdk25.coroutines.onClick

@ToolBar
class Hierarchy3Activity : ToolBarActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_hierarchy3)
        setTitle(intent.getStringExtra("title"))

        button.onClick {
            //将当前界面所有控件节点信息扫描出来
            val root=HierarchyNode(0,"#")
            hierarchyViewer(window.decorView,root,1)
            supportFragmentManager.
                    beginTransaction().
                    add(R.id.fragmentContainer,HierarchyFragment.newInstance(root)).commit()
        }
    }
